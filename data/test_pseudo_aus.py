...
def create_graph():
    ...
    for node in default_nodes:
        if node.name == "SYD":
            flights_used = random.randrange(int(0.4*flights_remaining), int(0.61*flights_remaining))
        else:
        ...
        for _ in range(0, flights_used):
            ...
            dest_node = random.choices(default_nodes, weights=[0.25, 0.15, 0.15, 0.1, 0.05, 0.05, 0.05, 0.05, 0.05, 0.05])[0]
            ...
            if departure_node in adj_list.get_all_nodes():
                adj_list.add_to_node(departure_node, dest_node.new_time_copy(departure_time + 2, current_node_id+1), current_flight_id)
            else:
                neighbours.append(dest_node.new_time_copy(departure_time + 2, current_node_id+1))
