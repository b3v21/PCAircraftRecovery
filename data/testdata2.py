...
CF_f = [[fd for fd in F if fd == AK_f[f] and fd != f] for f in F]

# Subset of itineraries compatible with a reasignment from an original itinerary p.
CO_p = [[i] for i,p in enumerate(P)] # Currently can only reschedule to itself
...
# zeta
theta = [
    [[[0 for _ in Y] for _ in P] for _ in P] for _ in Z
]

# Starting location of planes (binary) ((for t) for k)
tb = [random.sample([1] + [0 for _ in range(len(T)-1)],len(T)) for _ in K]
print(tb)
